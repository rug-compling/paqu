...
declare function local:output($node as element(node)) as xs:string* {
     local:conll(
          local:enhanced_dependencies(
            local:add_dependency_relations(
              local:add_features(
  ...
  for $href in doc($DIR)//doc/@href for $doc in doc($href)
    return
      ...
      (
        ...
        (: concat("# sent_id = ", replace(tokenize(document-uri($doc),"/")[last()],".xml","")), :)
        concat("# sent_id = ",replace($href,".xml","")),
        concat("# text = ", $doc/alpino_ds/sentence),
        local:output($doc/alpino_ds/node),
      "",""), "&#10;")
