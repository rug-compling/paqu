...
func main() {
	...
	if len(os.Args) != 4 || util.IsTerminal(os.Stdin) {
		fmt.Printf(`
Syntax: %s [-a] [-w] [-i] login_file id description < bestandnamen
...
`, os.Args[0])
	...
	login = strings.TrimSpace(os.Args[1])
	...
	if prefix == "wordrel" {
		util.CheckErr(fmt.Errorf("De naam 'wordrel' is gereserveerd"))
	}
	...
	rows, err := db.Query("SELECT `begin` FROM `" + PRE + "_c_" + prefix + "_deprel` LIMIT 0, 1;")
	if err == nil && rows.Next() {
		...
		if db_append {
			rows, err := db.Query("SELECT MAX(id) FROM " + PRE + "_c_" + prefix + "_arch")
			util.CheckErr(err)
			if rows.Next() {
			...
			rows, err = db.Query("SELECT MAX(id) FROM " + PRE + "_c_" + prefix + "_file")
			util.CheckErr(err)
			if rows.Next() {
			...
			fmt.Println("Verwijderen indexen uit " + PRE + "_c_" + prefix + "_deprel ...")
			db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_deprel
				DROP INDEX word,
				DROP INDEX lemma,
				DROP INDEX root,
				DROP INDEX postag,
				...
				DROP INDEX file,
				DROP INDEX arch;`)
			fmt.Println("Verwijderen indexen uit " + PRE + "_c_" + prefix + "_sent ...")
			db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_sent
				DROP INDEX file,
				DROP INDEX arch;`)
			fmt.Println("Verwijderen index uit " + PRE + "_c_" + prefix + "_file ...")
			db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_file
				DROP INDEX id`)
			fmt.Println("Verwijderen index uit " + PRE + "_c_" + prefix + "_arch ...")
			db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_arch
				DROP INDEX id;`)
		}
	}

	// Verwijder info over corpus uit database.
	// Wordt weer toegevoegd als aanmaak corpus correct is verlopen.
	_, err = db.Exec(`CREATE TABLE IF NOT EXISTS ` + PRE + `_info (
		id          varchar(128) NOT NULL,
		description varchar(128) NOT NULL,
		UNIQUE INDEX (id))
		DEFAULT CHARACTER SET utf8;`)
	util.CheckErr(err)
	_, err = db.Exec("DELETE FROM " + PRE + "_info WHERE `id` = \"" + prefix + "\";")
	util.CheckErr(err)
	...
	if !db_exists || db_overwrite {
		_, err := db.Exec(fmt.Sprintf(
			...
			prefix))
		util.CheckErr(err)
		_, err = db.Exec(`CREATE TABLE ` + PRE + "_c_" + prefix + `_deprel (
		...
		util.CheckErr(err)
		_, err = db.Exec(`CREATE TABLE ` + PRE + "_c_" + prefix + `_sent (
			arch int  NOT NULL,
			file int  NOT NULL,
			sent text NOT NULL)
			DEFAULT CHARACTER SET utf8
			DEFAULT COLLATE utf8_unicode_ci;`)
		util.CheckErr(err)
		_, err = db.Exec(`CREATE TABLE ` + PRE + "_c_" + prefix + `_file (
			id   int          NOT NULL,
			file varchar(260) NOT NULL)
			DEFAULT CHARACTER SET utf8;`)
		util.CheckErr(err)
		_, err = db.Exec(`CREATE TABLE ` + PRE + "_c_" + prefix + `_arch (
			id   int          NOT NULL,
	...
	*/
	...
	fmt.Println("Aanmaken indexen op " + PRE + "_c_" + prefix + "_deprel ...")
	_, err = db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_deprel
		ADD INDEX (word),
		ADD INDEX (lemma),
		ADD INDEX (root),
		ADD INDEX (postag),
	...
	fmt.Println("Aanmaken indexen op " + PRE + "_c_" + prefix + "_sent ...")
	_, err = db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_sent
		ADD INDEX (file),
		ADD INDEX (arch);`)
	util.CheckErr(err)

	fmt.Println("Aanmaken index op " + PRE + "_c_" + prefix + "_file ...")
	_, err = db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_file
		ADD UNIQUE INDEX (id)`)
	util.CheckErr(err)

	fmt.Println("Aanmaken index op " + PRE + "_c_" + prefix + "_arch ...")
	_, err = db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_arch
		ADD UNIQUE INDEX (id);`)
	util.CheckErr(err)
	...
	_, err = db.Exec(fmt.Sprintf(
		...
		prefix))
	util.CheckErr(err)
	_, err = db.Exec(`CREATE TABLE ` + PRE + "_c_" + prefix + `_word (
		word  varchar(128) NOT NULL,
	...
	*/
	...
	rows, err = db.Query("SELECT count(*), `word` FROM `" + PRE + "_c_" + prefix +
	...
	for idx, woord := range woorden {
		...
		*/
		rows, err := db.Query(fmt.Sprintf(
			"SELECT `lemma` FROM `"+PRE+"_c_"+prefix+"_deprel` WHERE `word` = %q GROUP BY `lemma`;",
			woord))
		util.CheckErr(err)
		for rows.Next() {
		...
		*/
		...
		rows, err = db.Query(fmt.Sprintf(
			"SELECT `root`,`postag` FROM `"+PRE+"_c_"+prefix+"_deprel` WHERE `word` = %q GROUP BY `root`,`postag`;",
			woord))
		util.CheckErr(err)
		for rows.Next() {
		...
		for _, root := range roots {
			rows, err := db.Query(fmt.Sprintf(
				"SELECT `lemma` FROM `"+PRE+"_c_"+prefix+"_deprel` WHERE `root` = %q AND `postag` = %q GROUP BY `lemma`;",
				root[0], root[1]))
			util.CheckErr(err)
			for rows.Next() {
				util.CheckErr(rows.Scan(&s))
	...
	fmt.Println("Aanmaken index op " + PRE + "_c_" + prefix + "_word ...")
	_, err = db.Exec(`ALTER TABLE ` + PRE + "_c_" + prefix + `_word
		ADD UNIQUE INDEX (word);`)
	util.CheckErr(err)

	// zet info over corpus in de database

	rows, err = db.Query("SELECT COUNT(*) FROM " + PRE + "_c_" + prefix + "_sent")
	util.CheckErr(err)
	if rows.Next() {
		var n int
		util.CheckErr(rows.Scan(&n))
		rows.Close()
		desc = fmt.Sprintf("%s \u2014 %s zinnen", desc, iformat(n))
	}
	_, err = db.Exec(fmt.Sprintf("INSERT %s_info VALUES (%q, %q);", PRE, prefix, desc))
	util.CheckErr(err)
...
func sent_buf_put(arch, file int, sentence string) {
	...
	if !buf_has_data[SENT] {
		...
		fmt.Fprintf(&buffer[SENT], `INSERT %s_c_%s_sent VALUES`, PRE, prefix)
	}
...
func deprel_buf_put(arch, file int, rel string, d *Deprel) {
	...
	if !buf_has_data[DPRL] {
		...
		( word,   lemma,   root,   postag, rel,   hword,   hlemma,   hroot,   hpostag, arch, file,   begin,   end,   hbegin,   hend,   mark) VALUES`, PRE, prefix)
	}
...
func word_buf_put(woord string, lemmas []string) {
	...
	if !buf_has_data[WORD] {
		...
		fmt.Fprintf(&buffer[WORD], `INSERT %s_c_%s_word VALUES`, PRE, prefix)
	}
...
func arch_buf_put(name string, n int) {
	...
	if !buf_has_data[ARCH] {
		...
		fmt.Fprintf(&buffer[ARCH], `INSERT %s_c_%s_arch VALUES`, PRE, prefix)
	}
...
func file_buf_put(name string, n int) {
	...
	if !buf_has_data[FILE] {
		...
		fmt.Fprintf(&buffer[FILE], `INSERT %s_c_%s_file VALUES`, PRE, prefix)
	}
...
func tags() {
	...
	rows, err := db.Query("SELECT `id` FROM `" + PRE + "_info`;")
	util.CheckErr(err)
	for rows.Next() {
		var val string
	...
	_, err = db.Exec("DROP TABLE IF EXISTS `" + PRE + "_postag`, `" + PRE + "_hpostag`, `" + PRE + "_rel`;")
	util.CheckErr(err)

	for _, t := range targets {
		_, err = db.Exec(`CREATE TABLE ` + PRE + `_` + t + ` (
			tag varchar(128))
			DEFAULT CHARACTER SET utf8;`)
	}

	for _, p := range prefixes {
		rows, err := db.Query("SELECT count(*) FROM `" + PRE + "_c_" + p + "_sent`;")
		util.CheckErr(err)
		for rows.Next() {
			var val int
			util.CheckErr(rows.Scan(&val))
		...
		for _, t := range targets {
			rows, err := db.Query("SELECT count(*),`" + t + "` FROM `" + PRE + "_c_" + p + "_deprel` group by `" + t + "`;")
			util.CheckErr(err)
			var key string
			var val int
			for rows.Next() {
	...
	for _, t := range targets {
		...
		sort.Strings(items)
		fmt.Fprint(&buf, "INSERT "+PRE+"_"+t+" VALUES (\"\")")
